	...
	<fieldset>
		...
		<ul>
			<li>
				...
				<input type="text" name="memberId" id="id" placeholder="4 ~12자의 영문 대소문자의 숫자로 작성해주세요."> 
			...
			<li>
				...
				<input type="password" name="memberPw" id="passwd"  placeholder="영문자,숫자,특수문자가 반드시 하나이상 포함된 6~20 범위의 문자로 작성해주세요.">
				<div id="passwdMsg" class="error">비밀번호를 입력해 주세요.</div>
				<div id="passwdRegMsg" class="error">비밀번호는 영문자,숫자,특수문자가 하나이상 포함된 6~20 범위의 문자로만 작성 가능합니다.</div>
			</li>
			<li>
				<label for="passwd">비밀번호 확인</label>
				<input type="password" name="memberPw2" id="repasswd" placeholder="같은 비밀번호를 작성해주세요."> 
			...
			<li>
				...
				<input type="text" name="memberName" id="name"  placeholder="홍길동">
				<div id="nameMsg" class="error">이름을 입력해 주세요.</div>
			</li>
				
			<li>
				<label for="email">이메일</label>
				<input type="text" name="memberEmail" id="email" placeholder="email@email.com">
...
$("#id").change(function() {
    ...
    $.ajax({
        type : 'get', // http 전송방식
        url : "<c:url value="/join/member_id_check"/>", //요청전송될 주소
        data : {"memberId":memberid}, // 요청시 전달할 데이터
        dataType : "text", 				// 응답 데이터 형식
        success : function(result) {// 결과를통해 정상적으로 요청받을경우
            if (result=="ok") {
            	idck = 1;
            	// 사용가능한 아이디
        ...
        error : function(error) { // 응답받지 못할경우
